...
void send_message(void) {
    uint8_t socket_num = 0; // Používáme socket 0
    uint16_t port = 5000;   // TCP/UDP port
    char message[] = "Hello, Ethernet!";

    // Otevření soketu jako TCP klient nebo server
    if (socket(socket_num, Sn_MR_TCP, port, 0) == 0) {
        connect(socket_num, (uint8_t[]){192, 168, 1, 200}, port); // Připojení na IP a port
        send(socket_num, (uint8_t*)message, strlen(message));
